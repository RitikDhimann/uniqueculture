...
const IndustrialMotion = () => {
  ...
  return (
    <section className="w-full relative  h-auto bg-white py-20 px-6 md:px-12 flex flex-col items-center">
      ...
      {/* Segmented Progress Bar (Non-SVG) */}
      <div className="mt-20 w-full max-w-[1000px] flex gap-4 h-6 items-end">
        {images.map((_, i) => (
          <button
            key={i}
          ...
          >
            <span
              className={`text-[10px] font-mono tracking-tighter transition-all duration-300 ${i === currentIndex ? 'text-secondary font-black' : 'text-accent opacity-40 group-hover/btn:opacity-100'
                }`}
            >
              0{i + 1}
            </span>
